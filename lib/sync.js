 ...
 * @param callback
 */
var checkIfNamespacePresentSync = function (nodeModulePath, appName) {
...
var registerSync = function (opts) {
    ...
    if (checkIfNamespacePresentSync(nodeModulePath, opts.namespace)) {
      isnamespacecloflict = true;
    }

    if (!isnamespacecloflict) {
    ...
    for (filename in config) {
        ...
        createSymLinkSync(path.resolve(nodeModulePath, opts.namespace),
            opts.basedir, filename, filepath);
    }
